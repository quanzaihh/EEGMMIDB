...
class CUBASpikingCNN(nn.Module):
    def __init__(self, spike_ts, params, record_neuron=None):
        ...
        # 初始化CBAM模块（根据插入位置选择）
        self.cbam_conv1 = cbam_sa.CBAM(gate_channels=64)  # conv1输出通道数64
        self.cbam_conv2 = cbam_sa.CBAM(gate_channels=128)  # conv2输出通道数128
        self.cbam_conv3 = cbam_sa.CBAM(gate_channels=256)  # conv3输出通道数256

        self.temp_conv1 = TemporalConvCUBALIFCell(3, [nn.Linear(256, 256, bias=True) for _ in range(3)],
                                                  pseudo_grad_ops_rec,
