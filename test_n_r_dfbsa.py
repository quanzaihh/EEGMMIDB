...
_OUT_DIR = Path.cwd() / "training_outputs_RF"
...
def test_accuracy(network, test_loader, device, if_output):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if if_output:
                [output, spike] = network(eeg_data, if_output)
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False,
                            sampler=val_sampler, num_workers=4, pin_memory=True)
    overall, class_acc, total_spike = test_accuracy(net,  val_loader, device, if_output=True)
