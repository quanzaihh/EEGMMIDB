...
if __name__ == "__main__":
    ...
    # Helper: run leave-k experiments across groups and report per-k accuracy
    def run_leave_k_experiments(groups, **train_kwargs):
        """Run train_network separately for each group in `groups` (each group is a list of subject ids).
        Returns a list of dicts with group and final validation accuracy."""
        results = []
        for i, group in enumerate(groups):
            logger.info('Running leave-k experiment %d / %d  -> validate subjects: %s', i + 1, len(groups), group)
            # ensure we pass the group as validate_subject_list and keep other kwargs
            kwargs = dict(train_kwargs)
            kwargs['validate_subject_list'] = group
            # Train model (may be time-consuming depending on epoch)
            model, metrics = train_network(dataset=kwargs.get('dataset', DATASET), network=kwargs.get('network', WrapCUBASpikingCNN),
                                           dataset_kwargs=kwargs.get('dataset_kwargs', ds_params), spike_ts=kwargs.get('spike_ts', SPIKE_TS),
                                           param_list=kwargs.get('param_list', PARAM_LIST), validate_subject_list=group,
                                           lr=kwargs.get('lr', [NEURON_LR, TS_LR, WT_LR]), weight_decays=kwargs.get('weight_decays', [2e-6,4e-6,1e-6]),
                                           batch_size=kwargs.get('batch_size', BATCH_SIZE), epoch=kwargs.get('epoch', 20),
                                           record_neuron=kwargs.get('record_neuron', None), seed=kwargs.get('seed', SEED),
                                           scheduler_type=kwargs.get('scheduler_type', 'plateau'), onecycle_max_lr=kwargs.get('onecycle_max_lr', None),
                                           onecycle_pct_start=kwargs.get('onecycle_pct_start', 0.3))
    ...
    # By default we run a single training for the first val_list group; to run full leave-k set RUN_LEAVE_K
    RUN_LEAVE_K = False
    if RUN_LEAVE_K:
        # run experiments across all defined val_list groups and save results
        run_leave_k_experiments(val_list, dataset=DATASET, dataset_kwargs=ds_params, spike_ts=SPIKE_TS, param_list=PARAM_LIST,
                               batch_size=BATCH_SIZE, epoch=20, lr=[NEURON_LR, TS_LR, WT_LR], weight_decays=[2e-6,4e-6,1e-6],
                               record_neuron=(0,0,0), seed=SEED)
    else:
        # default single run (same as previous behavior): run on first validation group
        train_network(dataset=DATASET, dataset_kwargs=ds_params, spike_ts=SPIKE_TS, param_list=PARAM_LIST,
