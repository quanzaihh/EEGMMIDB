...
from snn_n_r_dfbsa_Shiyan import WrapCUBASpikingCNN
...
def read_local_mat_file(file_path):
    """
    读取本地.mat文件（适用于MATLAB 7.3以下版本）

    参数:
        file_path: 本地.mat文件的路径（绝对/相对路径）
    返回:
        dict: 包含文件中所有变量的字典，失败返回None
    """
    # 先检查文件是否存在
    ...
    try:
        ...
        print("\n文件中包含的变量：")
        # 过滤系统自动生成的变量（__开头的）
        output= [k for k in mat_data.keys() if not k.startswith('__')]
        for idx, var_name in enumerate( output, 1):
            var_value = mat_data[var_name]
            print(f"  {idx}. 变量名：{var_name} | 数据形状：{var_value.shape} | 数据类型：{var_value.dtype}")

        return mat_data

    except Exception as e:
        # 若scipy读取失败，提示可能是高版本文件
        print(f"❌ scipy读取失败：{e}")
        print("提示：可能是MATLAB 7.3+版本的文件，请尝试使用h5py读取")
        return None


# Setup a module-level logger that writes to both stdout and a file in training_outputs/
_OUT_DIR = Path.cwd() / "training_outputs_LR"
...
def test_accuracy(network, test_loader, device, dfbsa_data, if_output):
    """Compute overall and per-class accuracy on test_loader.
    Returns: (overall_acc (float), class_acc (np.ndarray))
    """
    network.eval()

    dfbsa_a = np.asarray(dfbsa_data[0]['dfbsa_data']).reshape(-1)
    dfbsa_b = np.asarray(dfbsa_data[1]['dfbsa_data']).reshape(-1)
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            # Expecting dfbsa to be aligned as: (num_samples * spike_ts,) so slice by [batch * batch_size * spike_ts]
            start = (batch_idx * 64) * 160
            end = (batch_idx * 64 + eeg_data.shape[0]) * 160
            BB = dfbsa_a[start:end]
            dfbsa_np_1 = BB.reshape(eeg_data.shape[0], -1)
            dfbsa_tensor_1 = torch.from_numpy(dfbsa_np_1)

            CC = dfbsa_b[start:end]
            dfbsa_np_2 = CC.reshape(eeg_data.shape[0], -1)
            dfbsa_tensor_2 = torch.from_numpy(dfbsa_np_2)

            dfbsa_batch = torch.vstack((dfbsa_tensor_1 ,  dfbsa_tensor_2 ))

            if if_output:
                [output, spike] = network (eeg_data, dfbsa_batch, if_output)
                total_spike.append(spike)
            else:
                output = network(eeg_data, dfbsa_batch)
    ...
    network.train()
    if if_output:
        return overall, class_acc, total_spike
    else:
...
def train_network(dataset=EEGDataset2DLeftRight, network=WrapCUBASpikingCNN,
    ...
    for e in range(epoch):
        ...
        net.eval()
        acc, class_acc = test_accuracy(net, val_loader, device, dfbsa_data, if_output=False)
        net.train()

        # Save a checkpoint at epoch 44 so inference can load and use it directly
        if acc > test_best_acc:
            ...
            try:
                ...
                logger.info('Saved checkpoint at epoch %d to %s', e, str(ckpt_path))
            except Exception:
                logger.exception('Failed to save checkpoint at epoch %d', e)
...
def tensors_to_numpy(obj) :
    ...
    if isinstance(obj, list):
        return [tensors_to_numpy(x) for x in obj]
    if isinstance(obj, tuple):
        return tuple(tensors_to_numpy(x) for x in obj)
    if isinstance(obj, dict):
        return {k: tensors_to_numpy(v) for k, v in obj.items()}
    return obj

def to_object_array(lst):
    """显式构造 dtype=object 的 numpy 数组并逐项赋值，避免 NumPy 先尝试构造规则数组。"""
    lst = list(lst)  # 保证可迭代且有长度
    arr = np.empty(len(lst), dtype=object)
    for i, v in enumerate(lst):
        # 如果元素本身是 list，递归转为 object array（可选）
        if isinstance(v, list):
            arr[i] = to_object_array(v)
        else:
            arr[i] = v
    return arr
...
if __name__ == "__main__":
    ...
    local_mat_path_A = "/home/customer/Downloads/snn-eeg-main/eegmmidb/dfbsa_data_A_LR.mat"
    local_mat_path_B= "/home/customer/Downloads/snn-eeg-main/eegmmidb/dfbsa_data_B_LR.mat"

    dfbsa_data_A= read_local_mat_file(local_mat_path_A)
    dfbsa_data_B= read_local_mat_file(local_mat_path_B)
    dfbsa_data = [dfbsa_data_A, dfbsa_data_B]
    ...
    # Setup network
    net = WrapCUBASpikingCNN(SPIKE_TS, device, param_list=PARAM_LIST, dfbsa_data=dfbsa_data, record_neuron=None)
    # 注意检查这个路径
    net.load_state_dict(torch.load(_OUT_DIR / f"checkpoints/checkpoint_best.pth")['model_state_dict'])
    net = nn.DataParallel(net.to(device))

    # Dataset and transforms (train/val transforms should be provided at module-level or in dataset_kwargs)
    train_ds_kwargs = ds_params.copy()
    train_ds_kwargs.setdefault("transform", ToTensor())
    train_ds = DATASET(**train_ds_kwargs)

    val_ds_kwargs = ds_params.copy()
    val_ds_kwargs.setdefault("transform", ToTensor())
    val_ds = DATASET(**val_ds_kwargs)

    train_indices, val_indices = train_validate_split_subjects(train_ds, val_list[0])
    ...
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False,
                            sampler=val_sampler, num_workers=4, pin_memory=True)
    overall, class_acc, total_spike = test_accuracy(net,  val_loader, device, dfbsa_data, if_output=True)
    # total_spike = tensors_to_numpy(total_spike)
    # total_spike = to_object_array(total_spike)
    # savemat(_OUT_DIR / f"checkpoints/total_spike.mat", {"total_spike": total_spike}, do_compression=True)
    print('Overall validation Accuracy: %.3f %%' % (overall * 100))
    print('Accuracy for class 0: %.3f %%, 1: %.3f %%' % (float(class_acc[0]) * 100, float(class_acc[1]) * 100))
