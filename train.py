...
def train_network(dataset=EEGDataset2DLeftRight, network=WrapCUBASpikingCNN,
              #    dataset_kwargs=dict(), spike_ts=160, param_list=[],
                  ...
                  lr=[0.0001, 0.00001, 2*0.0001], weight_decays=[], batch_size=64, epoch=20, record_neuron = None):
    """
    ...
    # 训练结束后，触发神经元状态记录并画图（核心调用逻辑）
    if record_neuron is not None:
        print("\n" + "=" * 50)
        print("训练结束，开始记录并可视化神经元动态...")
        print("=" * 50)

    return net  # 可选：返回训练好的模型
...
if __name__ == "__main__":
    ...
    target_neuron = (0, 0, 0)  # 第0通道、第0行、第0列的神经元（可修改为其他坐标）
    ...
    train_network(dataset_kwargs=ds_params, spike_ts=SPIKE_TS, param_list=PARAM_LIST, dataset=DATASET,
                  ...
                  validate_subject_list=val_list[0], record_neuron =  target_neuron)
