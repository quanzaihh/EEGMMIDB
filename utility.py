...
# use the shared logger configured in train_n_r.py
...
def generate_and_save_all_slice_data(base_route, base_save_route, subject_id_list,
    ...
    try:
        ...
        logger.info("Directory %s created ...", base_save_route)
    except FileExistsError:
        logger.info("Directory %s already exists ...", base_save_route)
    for idx, subject_id in enumerate(subject_id_list, 0):
        subject_str = 'S%03d' % subject_id
        try:
            os.mkdir(base_save_route + subject_str + '/')
            logger.info("Directory %s created ...", base_save_route + subject_str + '/')
        except FileExistsError:
            logger.info("Directory %s already exists ...", base_save_route + subject_str + '/')
        for task in range(14):
            ...
            logger.info("Save Slice Norm Data and Labels %s", task_str)
...
def samples_per_class(labels):
    ...
    for la in labels:
        num_per_class[la] += 1
    logger.info("Number of samples per class: %s", num_per_class)
    return num_per_class
...
if __name__ == "__main__":
    route = '/Users/bytedance/workspace/yahui/eegmmidb/datasets/utsavsinghal2604/eeg-motorimagery-eegmmidb-v1-0-0/versions/1/physionet.org/files/eegmmidb/1.0.0/'
