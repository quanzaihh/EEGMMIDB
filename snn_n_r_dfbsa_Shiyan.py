...
class TemporalConvCUBALIFCell(nn.Module):
    def __init__(self, kernel, psp_func_list, pseudo_grad_ops, param):
        ...
        self.pseudo_grad_ops = pseudo_grad_ops
        self.cdecay, self.vdecay, self.vth, self.grad_win = param
        # Recording hook: when True and forward is called with train==False, input_data_list will be saved
        self.record_inputs = False
        # recorded_inputs will be a list where each element corresponds to one forward call and
        # contains a list of numpy arrays (one per temporal input in input_data_list)
    ...
    def forward(self, input_data_list, state, mask, train, if_output=False):
        ...
        volt = self.vdecay * pre_volt * (1. - pre_spike) + current
        output = self.pseudo_grad_ops(volt, self.vth, self.grad_win, mask)
        # If recording is enabled and we're in eval mode, capture the input_data_list
        if getattr(self, 'record_inputs', False) and not train:
            try:
                ...
                # don't interrupt forward if recording fails
                pass
        if if_output:
...
class CUBASpikingCNN(nn.Module):
    def __init__(self, spike_ts, params, dfbsa_data, record_neuron=None):
        """
        ...
        self.cbam_conv2 = cbam.CBAM(gate_channels=128) # conv2输出通道数128
...
# nn.Linear(256, 256, bias=True) for _ in range(3) 膜电位列表，输入到神经元
    ...
    def forward(self, input_data, states, dfbsa_data,  if_output=False):
        """
        ...
        for step in range(self.spike_ts):
            state = []
            D=dfbsa_data.shape[0]
            input_spike = input_data[:, :, :, :, step]
            c1_spike, c1_state = self.conv1(input_spike, c1_state, )
            c1_spike[:, 0, 0, 0] = dfbsa_data[0:int(D/2), step] #81.818
             # c1_spike[:, 0, 0, 0] = 1  82.483
            # c1_spike[:, 0, 0, 0] = 0 #82.262
            # c1_spike = deep_flatten(c1_spike)

            state.append([c1_state[0][:, 0, :, :], c1_state[1][:, 0, :, :], c1_state[2][:, 0, :, :]])
            # ====== 插入CBAM（新增代码） ======
            c1_spike_reshaped = c1_spike.view(batch_size, 64, -1)  # 展平空间维度
            c1_spike = self.cbam_conv1(c1_spike_reshaped).view(batch_size, 64, 8, 9)  # 注意力加权+��复形状
            # ===============================
            c2_spike, c2_state = self.conv2(c1_spike, c2_state)
            # Apply CBAM after conv2 (before pooling)
            ...
            # ====== 插入CBAM（conv3后） ======
            c3_spike_reshaped = c3_spike.view(batch_size, 256, -1)  # flatten spatial dims
            ...
            if if_output:
                ...
                A=dfbsa_data[int(D/2):int(D), step]
                tc1_spike[:, 0] = A
            else:
...
class WrapCUBASpikingCNN(nn.Module):
    def __init__(self, spike_ts, device, param_list, dfbsa_data, record_neuron=None):
        """
        :param spike_ts: spike timesteps
        :param device: device
        ...
        self.dfbsa_data = dfbsa_data
        self.snn = CUBASpikingCNN(spike_ts, param_list, dfbsa_data, record_neuron=record_neuron)

    def forward(self, input_data, dfbsa_data, if_output=False):
        """
        :param input_data: input EEG spike trains
        :return: output
        """
        ...
        if if_output:
            [output, spike] = self.snn(input_data, states, dfbsa_data, if_output)
            return [output, spike]
        else:
            output = self.snn(input_data, states, dfbsa_data, if_output)
            return output
