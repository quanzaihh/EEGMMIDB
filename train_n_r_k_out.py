...
from snn_n_r import WrapCUBASpikingCNN
# from snn_n_r_sa import WrapCUBASpikingCNN
# from snn_n_r_ca import WrapCUBASpikingCNN
import matplotlib
matplotlib.use('Agg')
...
_OUT_DIR = Path.cwd() / "training_outputs-k10_RF"
...
def train_network(dataset=EEGDatasetRightFeet, network=WrapCUBASpikingCNN,
    ...
    # return trained model and metrics so callers can inspect per-epoch performance
    metrics = {
...
def run_leave_k_experiments(groups, dataset, dataset_kwargs, spike_ts, param_list, batch_size=64, epoch=20,
                           lr=None, weight_decays=None, record_neuron=None, seed=None,
                           scheduler_type='plateau', onecycle_max_lr=None, onecycle_pct_start=0.3):
    """Run train_network separately for each group in `groups` (each group is a list of subject ids).
    Returns a list of dicts with group and final validation accuracy.
    This is module-level so it can be imported and called programmatically.
    """
    results = []
    for i, group in enumerate(groups):
        logger.info('Running leave-k experiment %d / %d  -> validate subjects: %s', i + 1, len(groups), group)
        try:
            model, metrics = train_network(dataset=dataset, network=WrapCUBASpikingCNN,
                                           dataset_kwargs=dataset_kwargs, spike_ts=spike_ts,
                                           param_list=param_list, validate_subject_list=group,
                                           lr=lr or [0.0001, 0.00001, 0.0002], weight_decays=weight_decays or [],
                                           batch_size=batch_size, epoch=epoch,
                                           record_neuron=record_neuron, seed=seed,
                                           scheduler_type=scheduler_type, onecycle_max_lr=onecycle_max_lr,
                                           onecycle_pct_start=onecycle_pct_start)
        except Exception as ex:
            logger.exception('Error during leave-k run for group %s: %s', group, ex)
            results.append({'group_index': i, 'validate_subjects': group, 'final_val_acc': None})
            continue

        final_acc = None
        try:
            final_acc = metrics['epoch_accs'][-1] if metrics['epoch_accs'] else None
        except Exception:
            final_acc = None

        results.append({'group_index': i, 'validate_subjects': group, 'final_val_acc': final_acc})
        # save intermediate results
        try:
            import csv
            out_csv = _OUT_DIR / 'leave_k_results.csv'
            write_header = not out_csv.exists()
            with open(out_csv, 'a', newline='') as f:
                writer = csv.writer(f)
                if write_header:
                    writer.writerow(['group_index', 'validate_subjects', 'final_val_acc'])
                writer.writerow([i, ' '.join(map(str, group)), final_acc])
        except Exception:
            logger.exception('Failed to append leave-k result to CSV')

    # Summary log
    logger.info('Leave-k experiments finished. Results:')
    for r in results:
        logger.info('Group %d subjects=%s final_val_acc= %s', r['group_index'], r['validate_subjects'], r['final_val_acc'])
    return results
...
if __name__ == "__main__":
    ...
    # For each group, compute the mean validation accuracy over the last 10 epochs and save to CSV.
    groups_to_run = val_list[0:10]
    results = []
    import csv
    out_csv = _OUT_DIR / 'leave_k_last10_avg.csv'
    write_header = not out_csv.exists()
    with open(out_csv, 'a', newline='') as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(['group_index_1based', 'validate_subjects', 'avg_last10_val_acc', 'last10_epoch_accs'])
        for idx, group in enumerate(groups_to_run, start=0):  # start=4 for 1-based group numbering
            logger.info('=== Running group %d: subjects=%s ===', idx, group)
            try:
                model, metrics = train_network(dataset=DATASET, dataset_kwargs=ds_params, spike_ts=SPIKE_TS, param_list=PARAM_LIST,
                                               batch_size=BATCH_SIZE, epoch=50, lr=[NEURON_LR, TS_LR, WT_LR], weight_decays=[2e-6, 4e-6, 1e-6],
                                               validate_subject_list=group, record_neuron=(0, 0, 0), seed=SEED)
            except Exception as ex:
                logger.exception('Training failed for group %d (%s): %s', idx, group, ex)
                writer.writerow([idx, ' '.join(map(str, group)), 'ERROR'])
                continue

            # compute average of last 10 epochs
            try:
                epoch_accs = metrics.get('epoch_accs', [])
                if len(epoch_accs) == 0:
                    avg_last10 = None
                else:
                    last10 = epoch_accs[-10:] if len(epoch_accs) >= 10 else epoch_accs
                    avg_last10 = float(np.mean(last10))
                # prepare semicolon-separated string of the last 10 accuracies for CSV
                try:
                    last10_str = ';'.join([f"{float(x):.6f}" for x in last10]) if len(epoch_accs) > 0 else ''
                except Exception:
                    last10_str = ''
                logger.info('Group %d average of last %d epochs val acc = %s; last10=%s', idx, min(10, len(epoch_accs)), avg_last10, last10_str)
                writer.writerow([idx, ' '.join(map(str, group)), avg_last10, last10_str])
                results.append({'group_index_1based': idx, 'validate_subjects': group, 'avg_last10_val_acc': avg_last10, 'last10': last10})
            except Exception as ex:
                logger.exception('Failed to compute/save avg for group %d: %s', idx, ex)
                writer.writerow([idx, ' '.join(map(str, group)), 'ERROR_COMPUTE'])
    # summary
    logger.info('Completed groups 4..10. Results written to %s', str(out_csv))
    # print summary to stdout for convenience
    for r in results:
        print(f"Group {r['group_index_1based']}: subjects={r['validate_subjects']} avg_last10_val_acc={r.get('avg_last10_val_acc')} last10={r.get('last10')}")
